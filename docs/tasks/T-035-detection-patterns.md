...
- **Test corpus safety**: Test strings must be clearly synthetic/fake. Use patterns like `AKIAIOSFODNN7EXAMPLE` (AWS's own example key), `ghp_` + `ABCDEF...` (assembled at runtime via `join()` to avoid push protection), etc. Never use real credentials.
