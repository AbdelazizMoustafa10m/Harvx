...
build_fix_prompt() {
  ...
  cat <<PROMPT
You are a senior Go engineer applying review fixes for Harvx.
...
3. Preserve Harvx conventions from AGENTS.md (Go/Cobra standards, deterministic behavior, no panics, error wrapping, tests where needed).
...
main() {
  ...
  if [[ "$DRY_RUN" == "true" ]]; then
    ...
    log_dim "[DRY-RUN] verification commands: go build ./cmd/harvx/ && go vet ./... && go test ./... && go mod tidy"
