#!/usr/bin/env bash
# Shared functions for Harvx review scripts.
...
collect_diff() {
  ...
  local high_risk_pattern='^(cmd/harvx/|internal/(cli|config|security|pipeline|server|workflows|diff|discovery|output|tokenizer)/|scripts/|\\.github/workflows/|go\\.mod$|go\\.sum$)'
...
build_full_review_prompt_inline() {
  ...
  patterns_content="$(read_context_file "$RULES_DIR/harvx-patterns.md")"
  ...
  cat <<PROMPT
You are running a Harvx pull-request review.
...
build_security_review_prompt_inline() {
  ...
  cat <<PROMPT
You are running a Harvx security review.
