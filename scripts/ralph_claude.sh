#!/usr/bin/env bash
#
# ralph_claude.sh -- Ralph Wiggum Loop for Harvx (Claude Code)
#
...
setup_logging() {
    mkdir -p "$LOG_DIR"
    LOG_FILE="$LOG_DIR/ralph-claude-$(date +%Y%m%d-%H%M%S).log"
...
run_ralph_loop() {
    ...
    log "=========================================="
    log "Ralph Loop Starting (Claude Code)"
    ...
    while [[ $iteration -lt $max_iterations ]]; do
        ...
        local prompt_file
        prompt_file=$(mktemp /tmp/ralph-claude-prompt-XXXXXX.md)
        ...
        local output=""
        local exit_code=0
        output=$(eval "cat '$prompt_file' | claude $claude_args" 2>&1) || exit_code=$?
...
usage() {
    cat <<'EOF'
Ralph Wiggum Loop for Harvx (Claude Code)
